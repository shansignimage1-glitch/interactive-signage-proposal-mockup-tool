...
export default function LegalCenter() {
  ...
  return <><nav className="fixed bottom-1 left-2 z-[120] flex gap-3 text-[11px] text-gray-400"><a href="#/privacy">Privacy</a><a href="#/terms">Terms</a><a href="#/support">Support</a></nav>{page && <div className="fixed inset-0 z-[250] bg-gray-950 text-white overflow-y-auto"><main className="max-w-2xl mx-auto p-6 md:p-12"><button onClick={() => { history.pushState(null, '', location.pathname); setPage(null); }} className="float-right p-2" aria-label="Close"><X/></button><h1 className="text-3xl font-bold mb-6">{copy[page].title}</h1><div className="space-y-4 text-gray-300 leading-7">{copy[page].body}</div><p className="mt-10 text-xs text-gray-500">Effective 7 August 2026</p></main></div>}</>;
}
